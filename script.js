...
async function initializeFirebase() {
  try {
    if (
      ...
      console.error(
        'Firebase Config está vazio ou incompleto. Certifique-se de que preencheu as suas credenciais.'
      );
      showMessage(
        'Erro de Configuração',
        'As credenciais do Firebase não foram carregadas corretamente. Verifique o seu código.'
      );
      return;
    }
    ...
    showMessage(
      'Erro',
      'Falha ao conectar ao serviço de autenticação. Tente novamente.'
    );
  }
}
...
const carouselProductDescription = document.getElementById(
...
const draggedImagesPreviewAdd = document.getElementById(
  'draggedImagesPreviewAdd'
);

const dropAreaEdit = document.getElementById('dropAreaEdit');
const imageUploadEdit = document.getElementById('imageUploadEdit');
const draggedImagesPreviewEdit = document.getElementById(
...
window.showConfirmModal = function (message, itemId, type = 'product') {
  confirmModalTitle.textContent = `Confirmar Exclusão de ${
    type === 'product' ? 'Produto' : 'Categoria'
  }`;
...
window.toggleAdminMenu = function () {
  document.getElementById('adminDropdownContent').style.display =
    document.getElementById('adminDropdownContent').style.display === 'block'
      ? 'none'
      : 'block';
};
...
function renderItems() {
  ...
  if (currentFilterCategory !== 'all') {
    filteredAndSearchedItems = filteredAndSearchedItems.filter(
      (item) => item.categoryId === currentFilterCategory
    );
  }
  ...
  let sortedCategoryGroups = Object.keys(itemsByCategory)
    .map((categoryId) => ({
      id: categoryId,
      name: itemsByCategory[categoryId].name,
      items: itemsByCategory[categoryId].items,
    }))
    ...
    if (categoryData.items.length > 0) {
      ...
      categoryData.items.forEach((item) => {
        ...
        if (item.price !== undefined && item.price !== null) {
          ...
          itemPrice.textContent = `R$ ${parseFloat(item.price)
            .toFixed(2)
            .replace('.', ',')}`;
          itemPrice.className =
            'item-price text-lg font-semibold text-gold-400 mt-1';
  ...
  if (!hasContent) {
    ...
    noItemsMessage.textContent =
      'Nenhum item disponível na categoria selecionada.';
      ...
      if (imageUrl.trim() !== '') {
        if (imageUrl.startsWith('https://firebasestorage.googleapis.com/')) {
          ...
          removeButton.style.cssText =
            'position: absolute; top: -5px; right: -5px; background: red; color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; justify-content: center; align-items: center; cursor: pointer; font-size: 14px;';
  ...
  try {
    ...
    for (const file of uploadedImageFilesAdd) {
      const storageRef = ref(
        storage,
        `product_images/${Date.now()}-${file.name}`
      );
    ...
    showMessage(
      'Erro',
      'Não foi possível adicionar o produto. Verifique sua conexão ou tente novamente.'
    );
  } finally {
...
window.updateProduct = async function () {
  ...
  try {
    ...
    for (const oldUrl of oldImages) {
      if (
        ...
        try {
          ...
          console.warn(
            `Erro ao remover imagem do Storage (${oldUrl}):`,
            deleteError
          );
        }
      }
    ...
    showMessage(
      'Erro',
      'Não foi possível atualizar o produto. Verifique sua conexão ou tente novamente.'
    );
  }
};
...
window.openImageCarouselModal = function (product) {
  ...
  if (product.price !== undefined && product.price !== null) {
    carouselProductPrice.textContent = `R$ ${parseFloat(product.price)
      .toFixed(2)
      .replace('.', ',')}`;
  } else {
...
window.closeImageCarouselModal = function () {
  ...
  imageCarouselModal.removeEventListener(
    'click',
    handleCarouselModalOutsideClick
  );
};
...
window.shareProductOnWhatsApp = function (name) {
  ...
  const productPrice =
    currentProductInCarousel.price !== undefined &&
    currentProductInCarousel.price !== null
      ? ` por R$ ${parseFloat(currentProductInCarousel.price)
          .toFixed(2)
          .replace('.', ',')}`
      : '';
  const productCode =
    currentProductInCarousel.code && currentProductInCarousel.code.trim() !== ''
...
function updateCarouselImage() {
  ...
  if (currentCarouselImages.length > 0) {
    ...
    carouselCounter.textContent = `${currentImageIndex + 1} / ${
...
window.addCategory = async function () {
  ...
  if (categoryName) {
    try {
      ...
      showMessage(
        'Sucesso',
        `Categoria "${categoryName}" adicionada com sucesso!`
      );
...
window.deleteProduct = async function (itemId) {
  ...
  try {
      ...
      for (const imageUrl of imagesToDelete) {
        if (imageUrl.startsWith('https://firebasestorage.googleapis.com/')) {
          try {
            ...
            console.warn(
              `Erro ao remover imagem do Storage (${imageUrl}):`,
              storageError
            );
          }
        }
    ...
    showMessage(
      'Sucesso',
      'Produto e imagens associadas removidos com sucesso!'
    );
  } catch (error) {
...
imageUploadAdd.addEventListener('change', (e) =>
  handleFiles(e.target.files, uploadedImageFilesAdd, draggedImagesPreviewAdd)
);
...
imageUploadEdit.addEventListener('change', (e) =>
  handleFiles(e.target.files, uploadedImageFilesEdit, draggedImagesPreviewEdit)
);
...
function renderFavoritesModal() {
  ...
  const favoritedProducts = items.filter((item) =>
  ...
  const total = favoritedProducts.reduce(
  ...
  shareButtonLeo.onclick = () =>
    shareFavoritesOnWhatsApp('leo', favoritedProducts, total);
  ...
  shareButtonJunior.onclick = () =>
    shareFavoritesOnWhatsApp('junior', favoritedProducts, total);
...
function removeFromFavorites(productId) {
  const removeButton = document.querySelector(
    `.remove-favorite-btn[data-id="${productId}"]`
  );
  if (removeButton) {
